...
erDiagram
    ...
    SEARCH_DOCUMENT {
        ...
        long version "For optimistic locking / CDC updates"
    }
...
3. **Версионирование (`version`):** Поле, хранящее `updated_at` (или LSN транзакции) из мастер-базы. Используется для разрешения конфликтов при асинхронной записи, чтобы предотвратить перезапись свежих данных старыми (опоздавшими) событиями из очереди.

---

## 7. Технические сценарии и потоки данных

Архитектура разделена на контур чтения (обработка пользовательских запросов) и контур записи (актуализация индекса).

### 7.1. Сценарий: Обработка поискового запроса (Read Path)
...
```
*Примечание:* Кэшируются только самые популярные комбинации запросов и фильтров (правило Парето: 20% запросов генерируют 80% трафика).

### 7.2. Сценарий: Обновление индекса (Write Path / CDC)
...
## 8. Архитектура системы (System Architecture)

### 8.1. Схема компонентов (High-Level Design)
...
### 8.2. Описание компонентов системы
...
### 8.3. Подход к фильтрации, сортировке и пагинации
...
### 8.4. Реализация подсказок (Autocomplete)
...
### 8.5. Масштабируемость под нагрузку (50k / 5k RPS)
...
### 8.6. Отказоустойчивость и актуальность (Resilience & Consistency)
...
* **Идемпотентность записи:** Поскольку Kafka гарантирует доставку *At-least-once*, возможны дубли сообщений. При вставке документа в OpenSearch в качестве идентификатора документа используется `product_id`, а в качестве параметра `version` передается таймстемп `updated_at` из PostgreSQL. OpenSearch автоматически отклоняет старые версии документов, если в индексе уже лежит более свежая запись.

### 8.7. Компромиссы архитектурного решения (Trade-offs)
